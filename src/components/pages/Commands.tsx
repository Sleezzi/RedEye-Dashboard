...
function Commands({ token, client }: {token: string, client: Client}) {
	...
	const save = async () => {
		try {
			const response = await fetch(`${client.url}/commands/disabled?id=${guild.id}`, {
				method: "POST",
				headers: {
						...
						Object.entries(guild.commands.custom).map(([name, message], index) => (
							<div key={name} className={styles.customCommand}>
								...
								<button className={styles.save} onClick={(e: any) => {
									...
									fetch(`${client.url}/commands/new?id=${guild.id}`, {
										method: "POST",
										headers: {
											authorization: token
										},
